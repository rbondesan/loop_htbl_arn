/*
  ...
  Plus modifications by Me, Roberto Bondesan
*/
...

{
  ...
  if (Prob.EigenvaluesFound()) {
    ...
    for (i=0; i<nconv; i++) {
      std::cout << " lambda[" << (i+1) << "]: " << std::setprecision (10) << real(Prob.Eigenvalue(i));
      if (imag(Prob.Eigenvalue(i))>=0.0) {
        std::cout << " + " << imag(Prob.Eigenvalue(i)) << " I" << std::endl;
      }
      else {
        std::cout << " - " << fabs(imag(Prob.Eigenvalue(i))) << " I" << std::endl;
      }
    }
    /* for (i=0; i<nconv; i++) { */
    /*   std::cout << "  lambda[" << (i+1) << "]: " << std::setprecision (10) << real(Prob.Eigenvalue(i)) << std::endl; */
    /*   std::cout << "  lambda[" << (i+1) << "]: " << std::setprecision (10) << Prob.Eigenvalue(i) << std::endl; */
    /* } */
    std::cout << std::endl;
  }
