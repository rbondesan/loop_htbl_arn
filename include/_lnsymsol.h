/*
   ...
   ARPACK Authors
      ...
      Plus addition of Eigenv_outfile
      by Me, Roberto Bondesan
*/
...
void Solution(MATRIX &A, ARluNonSymStdEig<FLOAT> &Prob)
{
  ...
  if (Prob.EigenvaluesFound()) {
    ...
    for (i=0; i<nconv; i++) {
      std::cout << " lambda[" << (i+1) << "]: " << std::setprecision (10) << Prob.EigenvalueReal(i);
  ...
  if (Prob.EigenvectorsFound()) {
	...
	for (j=0; j < n; j++)
	  {
	    std::cout << "x[" << (j+1) << "] = " 
		      << (Prob.RawEigenvector(i))[j] << std::endl;
	  }
	std::cout << "\n";
      }
#endif
...
/*
  Prints eigenvectors of nonsymmetric eigen-problems
  on outfile stream.
*/
template<class MATRIX, class FLOAT>
void Eigenv_outfile(MATRIX &A, ARluNonSymStdEig<FLOAT> &Prob, 
		    std::ofstream &outfile)
{
  ...
  outfile << "# vecs" << std::endl;

  for (i=0; i<nconv; i++) 
    {
      outfile << i << std::endl; //# of eigenvector
      for (j=0; j < n; j++)
	{
